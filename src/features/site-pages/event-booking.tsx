...
function createAttendees(ug: number, pg: number): Attendee[] {
  const createAttendee = (id: string, type: string, category: string): Attendee => ({ id, type, category, name: '', email: '', phone: '', whatsapp: '', regNo: '', aadhaar: '', fatherName: '', state: '', gender: '', college: '' });
  return [
    ...Array.from({ length: ug }, (_, index) => createAttendee(`ug-`, 'UG Early Bird', 'UG Scholar & Intern')),
    ...Array.from({ length: pg }, (_, index) => createAttendee(`pg-`, 'PG Early Bird', 'PG Scholar, Ph.D & Practitioner')),
  ];
}
...
function EventBookingContent() {
  ...
  const ug = parseInt(searchParams.get('ug') || '0', 10);
  const pg = parseInt(searchParams.get('pg') || '0', 10);
  ...
  const [attendees, setAttendees] = useState<Attendee[]>(() => createAttendees(ug, pg));

  useEffect(() => {
    if (ug === 0 && pg === 0) {
      router.push('/ayurved-yashobhoomi-2026');
    }
  }, [ug, pg, router]);
  ...
  const totalPrice = (ug * 1499) + (pg * 1999);

  const isFormValid = () => {
    if (!billingFirst.trim() || !billingEmail.trim() || !billingPhone.trim()) return false;
    for (const att of attendees) {
      if (!att.name.trim() || !att.email.trim() || !att.phone.trim() || !att.whatsapp.trim() || !att.aadhaar.trim() || !att.fatherName.trim() || !att.state.trim() || !att.gender || !att.college.trim()) {
        return false;
      }
    }
    return true;
  ...
  const handleCompletePayment = () => {
    if (payMethod === 'upi') {
      alert(`Booking Successful! Your Transaction ID: ${utr} has been submitted for verification. You will receive confirmation via Email/WhatsApp shortly.`);
    } else {
      alert("Redirecting to PayU gateway...");
      window.location.href = "https://u.payu.in/PAYUMN/qJM9c6eM5XLP";
    }
  };
    ...
    <>
      <style>{`
        ...
        .eb-back-btn {
          ...
          font-weight: 600;
          cursor: pointer;
          box-shadow: 0 4px 10px rgba(214, 55, 111, 0.3);
        ...
        .eb-attendee-card h3 {
          ...
          margin: 0 0 20px 0;
          padding-bottom: 10px;
          border-bottom: 1px dashed #ECE2CC;
        }
        .eb-summary-card {
          background: #fff;
          border-radius: 20px;
        ...
        .eb-proceed-btn:hover:not([disabled]) {
          background: #2563EB;
          transform: translateY(-2px);
        }
        .eb-proceed-btn:disabled {
          background: #cbd5e1;
          color: #94a3b8;
          cursor: not-allowed;
          box-shadow: none;
        ...
        .eb-pay-option.selected .eb-pay-radio::after {
          content: '';
          width: 10px;
          height: 10px;
          border-radius: 50%;
      ...
      <div className="eb-page-container">
        <div className="eb-page-wrap">
          
          <div className="eb-page-header">
            <h1>Event Booking</h1>
            
            <div className="eb-steps">
              <div className={`eb-step ${step >= 1 ? 'active' : ''}`}>
                <span className="eb-step-badge">{step > 1 ? '✓' : '1'}</span>
                <span className="eb-step-label">Attendee Details</span>
              </div>
              <div className="eb-step-line"></div>
              <div className={`eb-step ${step >= 2 ? 'active' : ''}`}>
                <span className="eb-step-badge">2</span>
                <span className="eb-step-label">Checkout & Payment</span>
              </div>
            </div>
          </div>
          ...
          <div className="eb-grid">
            ...
            <div className="eb-left-col">
              <button className="eb-back-btn" onClick={() => {
                if (step > 1) {
                  setStep(1);
                } else {
                  router.push('/ayurved-yashobhoomi-2026');
              ...
              {step === 1 ? (
                <div className="eb-form-container">
                  ...
                  <div className="eb-form-grid-2">
                    <div className="eb-field">
                      <label>First Name <span className="req">*</span></label>
                      <input type="text" placeholder="Enter First Name" value={billingFirst} onChange={e => setBillingFirst(e.target.value)} />
                    </div>
                    <div className="eb-field">
                      <label>Last Name</label>
                      <input type="text" placeholder="Enter Last Name" value={billingLast} onChange={e => setBillingLast(e.target.value)} />
                    </div>
                  </div>

                  <div className="eb-form-grid-2" style={{ marginTop: '16px' }}>
                    <div className="eb-field">
                      <label>Email <span className="req">*</span></label>
                      <input type="email" placeholder="Enter Email Address" value={billingEmail} onChange={e => setBillingEmail(e.target.value)} />
                    </div>
                    <div className="eb-field">
                      <label>Phone <span className="req">*</span></label>
                      <input type="text" placeholder="Enter Phone Number" value={billingPhone} onChange={e => setBillingPhone(e.target.value)} />
                    </div>
                  </div>

                  <h2 style={{ marginTop: '40px', paddingTop: '20px', borderTop: '1px solid #efeae2' }}>Attendee Details</h2>

                  {attendees.map((att, index) => (
                    <div key={att.id} className="eb-attendee-card">
                      <h3>Attendee {index + 1} ({att.category})</h3>
                      
                      <div className="eb-form-grid-2">
                        <div className="eb-field">
                          <label>Name <span className="req">*</span></label>
                          <input type="text" placeholder="Enter Full Name" value={att.name} onChange={e => handleAttendeeChange(index, 'name', e.target.value)} />
                        </div>
                        <div className="eb-field">
                          <label>Email <span className="req">*</span></label>
                          <input type="email" placeholder="Enter Email" value={att.email} onChange={e => handleAttendeeChange(index, 'email', e.target.value)} />
                        </div>
                      </div>
                      ...
                      <div className="eb-form-grid-2">
                        <div className="eb-field">
                          <label>Phone <span className="req">*</span></label>
                          <input type="text" placeholder="Enter Phone" value={att.phone} onChange={e => handleAttendeeChange(index, 'phone', e.target.value)} />
                        </div>
                        <div className="eb-field">
                          <label>WhatsApp Number <span className="req">*</span></label>
                          <input type="text" placeholder="Enter WhatsApp Number" value={att.whatsapp} onChange={e => handleAttendeeChange(index, 'whatsapp', e.target.value)} />
                        </div>
                      </div>

                      <div className="eb-form-grid-2">
                        <div className="eb-field">
                          <label>Registration / Internship / University Enrollment No.</label>
                          <input type="text" placeholder="Enrollment / Reg No." value={att.regNo} onChange={e => handleAttendeeChange(index, 'regNo', e.target.value)} />
                        </div>
                        <div className="eb-field">
                          <label>Aadhaar Number <span className="req">*</span></label>
                          <input type="text" placeholder="Enter 12 digit Aadhaar Number" value={att.aadhaar} onChange={e => handleAttendeeChange(index, 'aadhaar', e.target.value)} />
                        </div>
                      </div>
                      ...
                      <div className="eb-form-grid-2">
                        <div className="eb-field">
                          ...
                          <input type="text" placeholder="Enter Father's Name" value={att.fatherName} onChange={e => handleAttendeeChange(index, 'fatherName', e.target.value)} />
                        </div>
                        <div className="eb-field">
                          <label>State <span className="req">*</span></label>
                          <input type="text" placeholder="Enter State" value={att.state} onChange={e => handleAttendeeChange(index, 'state', e.target.value)} />
                        </div>
                      </div>
                      ...
                      <div className="eb-form-grid-2">
                        <div className="eb-field">
                          ...
                          <select value={att.gender} onChange={e => handleAttendeeChange(index, 'gender', e.target.value)}>
                        ...
                        <div className="eb-field">
                          ...
                          <input type="text" placeholder="Enter College Name" value={att.college} onChange={e => handleAttendeeChange(index, 'college', e.target.value)} />
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="eb-form-container">
                  ...
                  <p>Please select your preferred payment mode below:</p>
                  
                  <div className="eb-payment-options">
                    <div className={`eb-pay-option ${payMethod === 'upi' ? 'selected' : ''}`} onClick={() => setPayMethod('upi')}>
                      <div className="eb-pay-radio"></div>
                      <div>
                        <strong>Pay via UPI QR Code</strong>
                        <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: '#666' }}>Scan code with Google Pay, PhonePe, Paytm, or BHIM.</p>
                      </div>
                    </div>
                    
                    <div className={`eb-pay-option ${payMethod === 'online' ? 'selected' : ''}`} onClick={() => setPayMethod('online')}>
                      <div className="eb-pay-radio"></div>
                      <div>
                        <strong>Pay Online via Card / Net Banking (PayU)</strong>
                        <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: '#666' }}>Pay securely via Credit Card, Debit Card, Net Banking, or Wallet.</p>
                      </div>
                    </div>
                  </div>

                  {payMethod === 'upi' && (
                    <div className="eb-upi-section">
                      <h3>Scan this QR Code to Pay</h3>
                      <div className="eb-qr-container">
                        <img src={`https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=upi://pay?pa=vaibhav@okaxis%26pn=Ayurvedam%20Foundation%26am=${totalPrice}%26cu=INR`} alt="UPI QR Code" />
                      </div>
                      <div className="eb-qr-details">
                        <p style={{ fontSize: '16px', margin: '0 0 6px 0' }}>Amount Payable: <strong style={{ color: '#C9924E' }}>₹{totalPrice.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</strong></p>
                        <p style={{ fontSize: '14px', color: '#555', margin: '0' }}>UPI ID: <strong>info.ayurvedam@okaxis</strong></p>
                        
                        <div className="eb-field" style={{ marginTop: '24px', textAlign: 'left' }}>
                          <label>Enter UPI Transaction ID / UTR (12 Digits) <span className="req">*</span></label>
                          <input type="text" placeholder="Enter Transaction UTR" value={utr} onChange={e => setUtr(e.target.value)} />
                          <small style={{ color: '#777', marginTop: '4px', display: 'block' }}>Please enter the 12-digit UTR from your payment app receipt to confirm.</small>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <button 
                    className="eb-btn-submit" 
                    onClick={handleCompletePayment}
                    disabled={payMethod === 'upi' && !utr.trim()}
                  >
                    {payMethod === 'upi' ? 'Submit UTR & Complete Registration' : 'Proceed to PayU Gateway'}
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="eb-right-col">
              <div className="eb-summary-card">
                ...
                <p className="eb-summary-sub">UG & PG Scholars Registration</p>
                
                <div className="eb-summary-details">
                  <p style={{ margin: '0 0 6px 0' }}>📅 <b>December 23, 2026, 8:00 am</b></p>
                  <p style={{ margin: '0' }}>📍 <b>Govardhan Hall, Shri Prem Nagar Ashram, Haridwar</b></p>
                </div>

                <div className="eb-summary-divider"></div>

                <h4>Booking Summary</h4>
                <div className="eb-summary-items">
                  {ug > 0 && (
                    <div className="eb-summary-item">
                      <span>UG Early Bird x {ug}</span>
                      <span>₹{(ug * 1499).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                    </div>
                  )}
                  {pg > 0 && (
                    <div className="eb-summary-item">
                      <span>PG Early Bird x {pg}</span>
                      <span>₹{(pg * 1999).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="eb-summary-total-row">
                  <span>Total:</span>
                  <span className="eb-total-price">₹{totalPrice.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                </div>

                {step === 1 && (
                  <button className="eb-proceed-btn" onClick={handleProceed} disabled={!isFormValid()}>
                    Proceed to Payment
                  </button>
                )}
              </div>
...
`
  ...
  * { box-sizing: border-box; }
  ...
  .ayurvedam-home-content .hero-media .hero-video-frame.is-ready { opacity: 1; }
...
`
      ...
      <Suspense fallback={<div>Loading header...</div>}>
        <div dangerouslySetInnerHTML={{ __html: `<header class="elementor elementor-113 elementor-location-header" data-elementor-id="113" data-elementor-post-type="elementor_library" data-elementor-type="header">
...
<li class="menu-item menu-item-type-post_type menu-item-object-page menu-item-112"><a class="elementor-item" href="/contact-us" tabindex="-1">CONTACT US</a></li>
      ...
      {/* EVENT BOOKING CONTENT WRAPPER */}
      <Suspense fallback={<div style={{ padding: '80px', textAlign: 'center', background: '#FBF6EC', fontFamily: 'sans-serif' }}>Loading booking details...</div>}>
        <EventBookingContent />
      </Suspense>
      
      {/* FOOTER LAYOUT HTML */}
      <Suspense fallback={<div>Loading footer...</div>}>
        <div dangerouslySetInnerHTML={{ __html: `<footer class="elementor elementor-166 elementor-location-footer" data-elementor-id="166" data-elementor-post-type="elementor_library" data-elementor-type="footer">
